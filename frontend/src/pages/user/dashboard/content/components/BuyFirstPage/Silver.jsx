...
const SilverCard = ({ index }) => {
    const [showModal, setShowModal] = useState(false)
    return (
        <>
            <div className="card h-72 w-56 border border-[#C0C0C0] flex flex-col justify-center items-center gap-5" onClick={() => { setShowModal(true) }}>
            ...
            {
                showModal && <SilverModal setShowModal={setShowModal} index={index}/>
            }
        </>
    )
}

const SilverModal = ({setShowModal,index}) => {
    ...
    return (
        <div className="border border-black bg-[#333333] h-[40rem] w-[56rem] fixed inset-0 m-auto p-5">
            <div className="h-16 border-b border-white flex justify-between items-center  mx-auto">
                <span className='text-2xl text-white'>Ranked {index} in Silver Tier</span>
                <button className='h-8 w-8   cursor-pointer' onClick={() => { setShowModal(false) }}>
                    <img src="/icon/close.svg" className='fill-current' alt="" />
                </button>
            </div>
            <div className=" py-5 text-white overflow-y-scroll h-[calc(100%-4rem)] sc flex flex-col">
